...
Aunque el valor esperado es k=3, dado que el conjunto original tiene 3 clases, el mejor valor que se obtiene es k=2. Lo que tenemos que observar es el valor en le cual la gráfica se empieza a aplanar, es decir, donde cambia el codo. 
...
iris_data$class1[iris_data$class == 'Iris-setosa'] = 1
iris_data$class1[iris_data$class == 'Iris-versicolor'] = 2
iris_data$class1[iris_data$class == 'Iris-virginica'] = 3
# sepalLength y sepalWidth
plot(x[c(1,2)], col=iris3clusters$cluster)
plot(x[c(1,2)], col=iris_data$class1)


# petalLength y petalWidth
plot(x[c(3,4)], col=iris3clusters$cluster)
plot(x[c(3,4)], col=iris_data$class1)
```
...
El dataset `BrainpH` nos muestra los datos del tejido cerebral al momento de la muerte de un paciente. No existen valores erróneos, nulos o vacíos. Tenemos un marco de datos con 54 observaciones sobre las siguientes 5 variables:
- 1. pH - pH del tejido cerebral;
- 2. Sex - F o M;
- 3. Ethnicity - AfricanAmerican, Asian, Caucasian o PacificIslander;
- 4. Age - Edad a la que muere;
- 5. DeathType - Causa de muerte (Cardiac, Other o Suicide).

```{r}
df <-read.csv("https://vincentarelbundock.github.io/Rdatasets/csv/Stat2Data/BrainpH.csv", header=T, sep=",")
```
...
```
Veo que hay una variable que es como un indice, es la variable `X` que nos vamos a cargar
```{r}
df_clean <- df[,2:6]

summary(df_clean)
```
### Transformaciones deel DF
Por ejmplo, podemos transformar las categóricas en numéricas

```{r}
library(dplyr)

## Pasamos los atributos a factores numericos 1,2,3...

df_clean$Sex <- factor(ifelse(df_clean$Sex == "M", 1, 2))
df_clean$Ethnicity <- factor(ifelse(df_clean$Ethnicity == "Caucasian", 1,
                           ifelse(df_clean$Ethnicity == "Asian", 3, 
                                  ifelse(df_clean$Ethnicity=="AfricanAmerican",2,4))))
df_clean$DeathType <- factor(ifelse(df_clean$DeathType == "Cardiac",1,
                           ifelse(df_clean$DeathType == "Suicide", 3, 2)))

###### 
# Tarea pendiente: REALIZAR VISUALIZACION Y EDA
#####

## Convertimos a numericos

df_clean$Sex <- as.numeric(df_clean$Sex)
df_clean$Ethnicity <- as.numeric(df_clean$Ethnicity)
df_clean$Age <- as.numeric(df_clean$Age)
df_clean$DeathType <- as.numeric(df_clean$DeathType)

```

## Trabajamos ahora con la normalización solo con un dataset no-supervisado, eliminando así la var dependiente

```{r}
df_norm <- as.data.frame(scale(df_clean[,1:4])) # dejamos fuera la variable 
summary(df_norm)
```
### Como no conocemos le número de aclúster (suponemos que sean2 o 3 por la variable Deathtype que eliminamos hace un ratit...), robamos con varias valores de 1 a...
...
for (i in c(2,3,4,5,6,7,8,9,10))
{
  fitt <- kmeans(df_norm, i)
  y_clustert <- fitt$cluster
  skt <- silhouette(y_clustert, dt)
  result[i] <-  mean(skt[,3])
}

```
```{r}
plot(2:10, result[2:10], type = "o", col = "blue", pch = 0, xlab = "cluster number", ylab = "silueta")
```
Como no es claro si es un K = 3 o un K=6, repetimos la operacion con la evaluación del mejor modelo a través del resultado que nos ofrece la suma de los cuadrados de las distancias de los puntos de cada grupo con respecto a su centro (withinss) con la mayor separacion entre centro de grupos (betweenss).

Utilizamos la técnica del "codo"

```{r}
result <- rep(0,10)
for (i in c(2,3,4,5,6,7,8,9,10))
{
  fit <- kmeans(df_norm, i)
  result[i] <- fit$tot.withinss
}

plot(2:10, result[2:10], type = "o", col = "blue", pch = 0, xlab = "cluster number", ylab = "tot.withniss")

```
Realizamos la ultima comprobacion con el método **silueta media** y **Calinksi-Hasrabasz**

```{r}
library(fpc)
fit_ch_3 <- kmeansruns(df_norm, krange = 1:50, criterion = "ch")
fit_aws_3 <- kmeansruns(df_norm, krange = 1:10, criterion = "aws")
```
...
```
```{r}
plot(1:50,fit_ch_3$crit,type="o",col="blue",pch=0,xlab="Número de clústers",ylab="Criterio Calinski-Harabasz")

```
## Visualizamos la clasifciación

```{r}
df_clusters <- kmeans(df_norm,3)
matriz <- table(df_clusters$cluster,df_clean$DeathType)
matriz
```
```{r}
# Calculamos la precision de esta Clasificacion de k = 3
(matriz[1,1]+matriz[2,2]+matriz[3,3])*100/54 # 54 es el numero de observaciones. 
```
